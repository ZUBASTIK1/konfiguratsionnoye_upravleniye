...
term.insert("end", f"Эмулятор запущен. Команды (Этап 3): ls, cd, exit (заглушки)\nСессия: {user}@{host}\n\n")
term.insert("end",
    f"Параметры запуска:\n"
    f"VFS: {VFS_PATH}\n"
    f"Log: {LOG_PATH}\n"
...
def execute(cmd, args):
    if cmd == "exit":
        ...
        output = f"command: cd\nargs: {args}\nnote: директория не меняется (этап 3)"
        status = "ok"
    else:
