...
class Dir(VFSNode):
    def __init__(self, name, parent=None):
        ...
        self.children = {}
        self.mode = 0o755  # права по умолчанию для директорий
...
class File(VFSNode):
    def __init__(self, name, content, is_binary=False, parent=None):
        ...
        self.mode = 0o644  # права по умолчанию для файлов
...
term.insert("end", f"Эмулятор запущен. Команды (Этап 5): ls, cd, echo, cat, uniq, chmod, exit\n\n")
if VFS_PATH:
...
# --- Вспомогательная функция для отображения прав ---
def mode_to_str(mode, is_dir=False):
    perms = ""
    for who in [6, 3, 0]:  # user, group, other
        val = (mode >> who) & 0b111
        perms += "r" if val & 0b100 else "-"
        perms += "w" if val & 0b010 else "-"
        perms += "x" if val & 0b001 else "-"
    return ("d" if is_dir else "-") + perms
...
def execute(cmd, args):
    ...
    if cmd == "exit":
        ...
        lines = []
        for name, node in sorted(CWD.children.items()):
            if isinstance(node, Dir):
                perms = mode_to_str(node.mode, is_dir=True)
            else:
                perms = mode_to_str(node.mode, is_dir=False)
        ...
        if len(args) < 2:
            ...
            mode_str, name = args[0], args[1]
            if name in CWD.children:
                node = CWD.children[name]
                try:
                    new_mode = int(mode_str, 8)  # преобразуем в восьмеричное число
                    node.mode = new_mode
                    output = ""  # успешный chmod молчит
                except ValueError:
                    output = f"chmod: invalid mode: {mode_str}"
                    status = "error"
            else:
                output = f"chmod: cannot access '{name}': No such file or directory"
                status = "error"
