...
# Заглушки (чтобы при импорте pytest не падал)
cfg = {}
VFS_PATH = None
LOG_PATH = None
SCRIPT_PATH = None
...
def execute(cmd, args):
    ...
    if cmd == "exit":
        ...
        for name, node in sorted(CWD.children.items()):
            perms = mode_to_str(node.mode, isinstance(node, Dir))
...
if __name__ == "__main__":
    ...
    term.configure(state="normal")
    term.insert("end", f"Эмулятор запущен. Команды (Этап 6): ls, cd, echo, cat, uniq, chmod, exit\n\n")
    if VFS_PATH:
